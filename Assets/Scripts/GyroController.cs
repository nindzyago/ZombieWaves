...
public class GyroController : MonoBehaviour 
{
	...
	protected void Update() 
	{
		if (!gyroEnabled) 
			return;
		//float prevAngle = transform.rotation.z;
		Quaternion newrotation = Quaternion.Slerp (transform.rotation,
		                                       cameraBase * (ConvertRotation (referanceRotation * Input.gyro.attitude) * GetRotFix ()), lowPassFilterFactor);
		//                                           cameraBase * (ConvertRotation (referanceRotation * Input.acceleration) * GetRotFix ()), lowPassFilterFactor);
		angle = newrotation.eulerAngles.z;

		if (!((angle > 45) & (angle < 315))) {
					transform.rotation = newrotation; 
					transform.rotation = Quaternion.Euler (0, 0, angle);
				}
		//transform.rotation.Set(x, y, newrotation.z, newrotation.w);
